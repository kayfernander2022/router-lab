...
// Finds one stock that matches the stock symbol
export function getStockLoader({params}){
  const stock = stockData.filter(s => {
    if(s.symbol.toLowerCase() === params.symbol.toLowerCase()){
      return s;
    }
  })[0];//if there is more then one, give me the first one.

  return { stock };//sending to stock.js
}